...
import { CategoryChannel, Channel, CommandInteraction, TextChannel } from 'discord.js';
...
export const slashCommand: SlashCommand = {
	...
	commandFunction: async (i: CommandInteraction) => {
		await i.reply({ ephemeral: true, content: 'This command has been disabled for safety' }).catch(console.log);
		...
		// try {
		// 	const channel = i.options.getChannel('channel', true) as Channel;
		// 	const category: CategoryChannel = (!channel.isText() ? channel : (channel as TextChannel).parent) as CategoryChannel;
